...
2. **Specifier Dispatch — `format_handler()`**
   - Retrieves the correct handler function from the `specifier_table` using the specifier.
...
This design allows new specifiers to be added with minimal coupling, just by registering them in `specifier_table` and defining a handler function.
